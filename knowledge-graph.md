...
| 已实现 | 窗口壳（WCO 标题栏 / 托盘 / 1024×700）+ 侧栏与 keepalive 占位导航 + 能力核骨架 + 窄 IPC + 壳快照/偏好 JSON 持久化（原子写 + version 迁移）+ 明暗双主题 + 双语界面（zh-CN / en，轻量 `src/shared/i18n`）+ 首选项 + 关于弹层 + 分栏文件浏览器能力（`fs-browser`）+ 文件预算 / 能力包契约 / ESLint / Vitest 质量门 + Windows 打包 + 主进程日志 |
| 占位 | 尚无真实业务数据；Projects/任务为演示树；检查更新/帮助仍占位；远程埋点未做 |
| 质量门槛 | 见 §3 |
