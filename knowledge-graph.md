...
| 已实现 | 窗口壳（WCO 标题栏 / 托盘 / 1024×700）+ 侧栏与 keepalive 占位导航 + 能力核骨架 + 窄 IPC + 壳快照/偏好 JSON 持久化 + 明暗双主题（菜单 segment + WCO 同步）+ 首选项 General（字体/五档字号）与 System（关闭隐藏托盘/退出）+ 关于弹层 + 文件预算门禁 + Windows 打包 |
| 占位 | 尚无真实业务数据；Projects/任务为演示树；语言偏好已落盘但界面文案尚未跟 locale 切换；检查更新/帮助仍占位；埋点未做 |
| 质量门槛 | 见 §3 |
...
| 02 | `knowledge/02-modules-main.md` | 主进程：Windows 校验、路径隔离、`BrowserWindow`、IPC handle | 改窗口、落盘路径、主进程生命周期、新 IPC 实现 |
...
| 05 | `knowledge/05-patterns-core.md` | 核心心智：三进程边界 / 共享类型 / 禁止事项 | 新建模块时判断代码该落哪一端 |
| 06 | `knowledge/06-patterns-state.md` | 状态：Zustand selector / 何时 IPC / 不写系统 AppData | 加客户端状态、持久化、跨窗口数据 |
| 07 | `knowledge/07-patterns-build.md` | 工程：electron-vite / electron-builder / tsconfig paths / Tailwind v4 / confine | 改构建、打包、TS、缓存隔离、排障工程问题 |
...
> 即 `tsc --noEmit` 分别跑 `tsconfig.node.json` 与 `tsconfig.web.json`，再跑文件预算（硬门槛 500 行）。未配 ESLint，提交前以 typecheck 为准。
...
- 改构建、Tailwind、TS、confine、electron-builder → `07-patterns-build.md`
