...
当前包：`shell`（快照 + 最大化状态）、`preferences`（偏好 JSON）、`app-info`（关于用的 name/version）、`fs-browser`（根列 / 卷 / 单层列举+缓存 / 详情 / 路径解析 / 文件图标含 `.lnk`；**不做**默认整盘 MFT；渲染侧可复用 `FileBrowser`，项目目录 Popover 等场景嵌入）。
